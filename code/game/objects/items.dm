...
/obj/item/examine(mob/user) //This might be spammy. Remove?
	...
	if(item_flags & (ITEM_CAN_BLOCK | ITEM_CAN_PARRY))
		...
		. += "[src] имеет потеницал к блокированию и/или паррированию. <a href='byond://?src=[REF(data)];name=[strip_html(name)];block=[item_flags & ITEM_CAN_BLOCK];parry=[item_flags & ITEM_CAN_PARRY];render=1'>\[Show Stats\]</a>"
