...
/obj/item/modular_computer/proc/alert_call(datum/computer_file/program/requester, alerttext, sound = 'sound/machines/twobeep_high.ogg')
	...
	visible_message(span_notice("The [src] displays a [requester.filedesc] notification: [alerttext]"))
	var/mob/living/holder = loc
	if(istype(holder))
		to_chat(holder, "[icon2html(src)] [span_notice("The [src] displays a [requester.filedesc] notification: [alerttext]")]")
