...
// Кнопки для амбиций и их отображение
	...
	if(LAZYLEN(ambition_objectives))
		...
		for(var/datum/ambition_objective/objective in ambition_objectives)
			...
			output += "<a href='byond://?src=[REF(src)];amb_delete=[REF(objective)]'>Удалить</a> " // Удалить амбицию
			output += "<a href='byond://?src=[REF(src)];amb_completed=[REF(objective)]'>" // Определить завершенность амбиции
			output += "<font color=[objective.completed ? "green" : "red"]>[objective.completed ? "Передумать" : "Выполнить"]</font>"
			output += "</a>"
			output += "<br>"
			amb_count++

	output += "<a href='byond://?src=[REF(src)];amb_add=1'>Добавить амбицию</a><br><br>"
	output += "</UL>"
...
/datum/mind/proc/show_editable_objectives_and_ambitions()
	...
	for(var/a in antag_datums)
		...
		if(is_admin)
			output += " <a href='byond://?src=[REF(antag_datum.owner)];obj_add=[REF(antag_datum)];ambition_panel=1'>Add Objective</a>"
		output += "<ul>"
		if(!length(antag_datum.objectives))
			...
			for(var/count in 1 to length(antag_datum.objectives))
				...
				if(self_mind)
					output += " <a href='byond://?src=[REF(antag_datum.owner)];req_obj_delete=[REF(objective)];target_antag=[REF(antag_datum)]'>Request Remove</a> <a href='byond://?src=[REF(antag_datum.owner)];req_obj_completed=[REF(objective)];target_antag=[REF(antag_datum)]'><font color=[objective.completed ? "green" : "red"]>[objective.completed ? "Request incompletion" : "Request completion"]</font></a><br>"
				if(is_admin)
					output += " <a href='byond://?src=[REF(antag_datum.owner)];obj_edit=[REF(objective)];target_antag=[REF(antag_datum)]'>Edit</a> <a href='byond://?src=[REF(antag_datum.owner)];obj_panel_delete=[REF(objective)];target_antag=[REF(antag_datum)]'>Remove</a> <a href='byond://?src=[REF(antag_datum.owner)];obj_panel_complete_toggle=[REF(objective)];target_antag=[REF(antag_datum)]'><font color=[objective.completed ? "green" : "red"]>[objective.completed ? "Mark as incomplete" : "Mark as complete"]</font></a><br>"
		output += "</ul>"
		if(is_admin)
			output += "<a href='byond://?src=[REF(antag_datum.owner)];obj_announce=1;ambition_panel=1'>Announce objectives</a><br>"
		output += "<br><i><b>Requested Objective Changes</b></i>:"
		if(self_mind)
			output += " <a href='byond://?src=[REF(antag_datum.owner)];req_obj_add=1;target_antag=[REF(antag_datum)]'>Request objective</a>"
		output += "<ul>"
		if(!LAZYLEN(antag_datum.requested_objective_changes))
			...
			for(var/uid in antag_datum.requested_objective_changes)
				...
				switch(obj_request)
					if(REQUEST_NEW_OBJECTIVE)
						...
						if(self_mind)
							output += " <a href='byond://?src=[REF(antag_datum.owner)];req_obj_cancel=[uid];target_antag=[REF(antag_datum)]'>Cancel Request</a>"
						if(is_admin)
							output += " <a href='byond://?src=[REF(antag_datum.owner)];req_obj_accept=[REF(antag_datum)];req_obj_id=[uid]'>Accept</a> <a href='byond://?src=[REF(antag_datum.owner)];req_obj_edit=[REF(antag_datum)];req_obj_id=[uid]'>Edit</a> <a href='byond://?src=[REF(antag_datum.owner)];req_obj_deny=[REF(antag_datum)];req_obj_id=[uid]'>Deny</a>"
					if(REQUEST_DEL_OBJECTIVE)
						...
						if(self_mind)
							output += " <a href='byond://?src=[REF(antag_datum.owner)];req_obj_cancel=[uid];target_antag=[REF(antag_datum)]'>Cancel Request</a>"
						if(is_admin)
							output += " <a href='byond://?src=[REF(antag_datum.owner)];req_obj_accept=[REF(antag_datum)];req_obj_id=[uid]'>Accept</a> <a href='byond://?src=[REF(antag_datum.owner)];req_obj_deny=[REF(antag_datum)];req_obj_id=[uid]'>Deny</a>"
			...
			output += "</ul><br>"
			if(self_mind)
				output += "<a href='byond://?src=[REF(src)];req_obj_ping=1'>Ping the admins</a><br>"
			if(is_admin)
				output += "<a href='byond://?src=[REF(src)];req_obj_ping_cd_clear=1'>Clear ping cooldown</a><br>"
	output += "<br><b>[current.real_name]'s Ambitions:</b>"
	if(LAZYLEN(ambitions) < CONFIG_GET(number/max_ambitions))
		output += " <a href='byond://?src=[REF(src)];add_ambition=1'>Add Ambition</a>"
	output += "<ul>"
	if(!LAZYLEN(ambitions))
		output += "<li><i><b>NONE</b></i>"
	else
		for(var/count in 1 to LAZYLEN(ambitions))
			output += "<li><B>Ambition #[count]</B> (<a href='byond://?src=[REF(src)];edit_ambition=[count]'>Edit</a>) (<a href='byond://?src=[REF(src)];remove_ambition=[count]'>Remove</a>):<br>[ambitions[count]]"
	output += "</ul><br>(<a href='byond://?src=[REF(src)];refresh_obj_amb=1'>Refresh</a>)"
	return output.Join()
...
/datum/mind/Topic(href, href_list)
	...
	if (href_list["refresh_obj_amb"])
		...
		message_admins("<span class='adminhelp'>[ADMIN_TPMONTY(usr)] has requested a review of their objective changes. (<a href='byond://?_src_=holder;[HrefToken(TRUE)];ObjectiveRequest=[REF(src)]'>RPLY</a>)</span>")
		do_edit_objectives_ambitions()
		return
