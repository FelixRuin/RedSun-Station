 ...
 */
#define CAN_STEP(cur_turf, next) (next && !next.density && !(simulated_only && SSpathfinder.space_type_cache[next.type]) && !cur_turf.LinkBlockedWithAccess(next,requester, id) && (next != avoid))
...
/turf/proc/LinkBlockedWithAccess(turf/destination_turf, requester, ID)
	if(destination_turf.x != x && destination_turf.y != y) //diagonal
		...
		for(var/first_step_direction in list(first_step_direction_a,first_step_direction_b))
			...
			var/way_blocked = midstep_turf.density || LinkBlockedWithAccess(midstep_turf,requester,ID) || midstep_turf.LinkBlockedWithAccess(destination_turf,requester,ID)
			if(!way_blocked)
				return FALSE
		return TRUE
